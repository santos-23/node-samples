...
const KAFKA_CONSUME_COUNT = Number("KAFKA_CONSUME_COUNT" in process.env ? process.env.KAFKA_CONSUME_COUNT : 1);

const protoPath = './proto/pool.proto';
const protoType = 'pool.Share';

const FILE_OFFSET = ROOT_DIR + `/${KAFKA_TOPIC}_offset.data`;
console.log('root path ', FILE_OFFSET);
let OFFSET = 0;
let values_arr = [];
let values_str;
...
consumer.on("ready", async (info, metadata) => {
    ...
    if (fileExist) {
        ...
        consumer.offsetsForTimes(
            [ {topic: KAFKA_TOPIC, partition: KAFKA_PARTITION, offset: 1714003200000 } ],
            10000,
            (err,offsets)=>{
                console.log(offsets)
...
async function processMessage(data) {
    // console.log("offset : ",data.offset," timestamp : ",data.timestamp)
    try {
        ...
        const decoded = decodedMessage.toJSON();
        // console.log(decoded)
        if(decoded.shareTime >= 1713777743 || data.timestamp >= 1713777743241){
            console.log("offset : ",data.offset," timestamp : ",data.timestamp)
            console.log("specified timestamp reached")
            console.log(decoded)
        ...
        if((decoded.shareTime >= 1714003200 && decoded.shareTime <= 1713810599) || (decoded.shareTime > 1713810599 && values_arr.length != 0)){

...
consumer.on("ready", async (info, metadata) => {
    ...
    if(await fs.promises.stat(parentDirectory+`/offset-${TOPIC}.txt`).then((d)=>true).catch((e)=> false)){
        ...
        consumer.queryWatermarkOffsets( TOPIC, PARTITION, 1000, async (err, offsets) => {
            ...
            console.log(offsets)
            fs.writeFile(parentDirectory+`/offset-${TOPIC}.txt`, JSON.stringify({offset:offsets.lowOffset}),"utf-8", (error)=>{
                if (error) {
                    console.log('An error has occurred ', error);
                    return;
                }
                console.log('Offset data written to this file --- ',offsets.lowOffset);
            })
            consumer.assign([{topic: TOPIC, partition: PARTITION, offset: offsets.lowOffset}]);
            consumer.consume()
        });
...
function write_file(param_OFFSET){
    console.log(param_OFFSET)
    fs.readFile(parentDirectory+`/offset-${TOPIC}.txt`,'utf-8',(error,data1) => {
        if(error){
            console.log(error)
        }
        data1 = JSON.parse(data1);
        if(param_OFFSET > data1.offset){
            fs.writeFile(parentDirectory+`/offset-${TOPIC}.txt`, JSON.stringify({offset:param_OFFSET}),"utf-8", (error)=>{
                if (error) {
                    console.log('An error has occurred ', error);
                    return;
                }
                console.log('Offset data written to this file --- ',param_OFFSET);
            })
        }
    });
...
function exitHandler(options, exitedBy) {
    ...
    try {
        ...
        // console.log('Data written successfully before exit.... Data: ',OFFSET);
        if(options.exit){
            consumer.disconnect();
            console.log("Exited by ",exitedBy)
